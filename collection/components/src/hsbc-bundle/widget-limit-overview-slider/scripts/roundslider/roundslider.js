...
(function ($, window, undefined) {
    ...
    RoundSlider.prototype = {
        ...
        _render: function () {
            ...
            this.bottomCurveLeft = createElement("div.rs-bottom-curve left rs-range-color");
            this.bottomCurveRight = createElement("div.rs-bottom-curve right rs-path-color");
            this.container.append(this.innerContainer.append(this.block));
            this.control.addClass("rs-control").empty().append(this.container);
            this.control.append(this.bottomCurveLeft);
            this.control.append(this.bottomCurveRight);
        ...
        _createLayers: function () {
            ...
            if (this._rangeSlider || this._showRange) {
                ...
                this.block5 = path.clone().addClass("rs-default-color").removeClass("rs-transition").css("opacity", "0").rsRotate(start);
                this.block4 = path.addClass("rs-path-color").css({ "opacity": "1", "z-index": "1" }).rsRotate(start - 180);


                this.block.append(this.block1, this.block2, this.block5, this.block3, this.block4).addClass("rs-split");
            }
        ...
        _createHandle: function (index) {
            ...
            var midMaxAngle = this._limitAngle(this._valueToAngle(this.options.midMax || 0));
            ...
            this.bar = bar;
            this.barFake = this.bar.clone().css({"opacity": "0", "z-index":"-1"}).addClass('barFake');
            if(midMaxAngle > 0){
                this.barFake.rsRotate(midMaxAngle);
                this.container.append(this.barFake);
            }
            this._active = index;
        ...
        _refreshHandle: function () {
            ...
            this.bottomCurveLeft.css({ height: h, width: w, "margin-bottom":  -h / 2 + "px"});
            this.bottomCurveRight.css({ height: h, width: w, "margin-bottom":  (-h / 2 +2) + "px"});
        ...
        _handles: function () {
            return this.container.children("div.rs-bar").not('.barFake').find(".rs-handle");
        },
        _activeHandleBar: function (index) {
            index = (index != undefined) ? index : this._active;
            return $(this.container.children("div.rs-bar").not('.barFake')[index - 1]);
        },
        ...
        _raiseEvent: function (event) {
            ...
            if (preValue !== this.options.value) {
                this._roundOffValue();
                this._popSetPosition('current');// current, pre
        ...
        _elementDown: function (e) {
            ...
            if ($target.hasClass("rs-handle") && !$target.hasClass('barFake')) {
                this._handleDown(e);
            }
            else {
                ...
                if (distance >= innerDistance && distance <= outerDistance) {
                    e.preventDefault();
                    var handle = this.control.find(".rs-handle.rs-focus").not('.barFake'), angle, value;
                    //if (handle.length == 0)
        ...
        _handleFocus: function (e) {
            ...
            if(!$target.hasClass('barFake')){
                ...
                this.control.find("div.rs-bar").not('.barFake').css("z-index", "7");
        ...
        _changeSliderValue: function (value, angle) {
            var oAngle = this._oriAngle(angle), lAngle = this._limitAngle(angle), midMaxAngle;
            midMaxAngle = this._limitAngle(this._valueToAngle(this.options.midMax || 0));
            if (!this._rangeSlider && !this._showRange) {
                ...
                if(midMaxAngle > lAngle){
                    this.block3.rsRotate(midMaxAngle);
                    this.block5.css("opacity", 1).rsRotate(lAngle);
                }
                else{
                    this.block3.rsRotate(lAngle);
                    this.block5.css("opacity", 0);
                }
        ...
        _tooltipValue: function (value, index) {
            ...
            var displayTip = (val != null && typeof val !== "boolean") ? val : value;
            if(this.options.currencySymbol !== null){
                displayTip = currencyFormat(displayTip, this.options.currencySymbol);
            }
            return displayTip;
        },
    ...
    function currencyFormat(currency, symbol) {
        return '<sup>'+symbol+'</sup> ' +  '<span>' + currency.toFixed(0).replace(/./g, function(c, i, a) {
    ...
    $.fn[pluginName].prototype._popUpCreate = function(){
        var _this = this,
            containerOffset = _this.container.position(),
            containerOffsetRight = containerOffset.left + parseFloat(_this.container.width()) + 15,
            containerOffsetBottom = containerOffset.top + parseFloat(_this.container.height());

        _this.popUpEnd = createElement("div.rslider-popup end");
        //_this.control.insertAfter(_this.popUpEnd);
        _this.container.append(_this.popUpEnd);

        _this.popUpEnd.html( currencyFormat(_this.options.max, _this.options.currencySymbol) ).addClass('right');

        containerOffsetBottom -= (parseFloat(_this.popUpEnd.height())/2);
        _this.popUpEnd.css({position:'absolute', top: containerOffsetBottom, left: containerOffsetRight, 'z-index': 50});

        if(_this.options.midMax > 0){
            _this.popUpMid = createElement("div.rslider-popup mid");
            //_this.control.insertAfter(_this.popUpMid);
            _this.container.append(_this.popUpMid);
            _this._popSetPosition('pre');
        }

        _this.popUp = createElement("div.rslider-popup");
        //_this.control.insertAfter(_this.popUp);
        _this.container.append(this.popUp);
    };

    $.fn[pluginName].prototype._popSetPosition = function(type){ // current, pre
        var _this = this, elemObj = {
            handle : ''  ,
            tooltip: '',
            value: ''
        };
        switch(type){
            ...
            case 'pre':     elemObj.handle = 'barFake';
                elemObj.tooltip = 'popUpMid';
                elemObj.value = 'midMax';
                break;
            case 'current': elemObj.handle = 'bar';
                elemObj.tooltip = 'popUp';
                elemObj.value = 'value';
                break;
        }

        _this[elemObj.tooltip].html( currencyFormat(_this.options[elemObj.value], _this.options.currencySymbol) );

        setTimeout(function(){
            var handle = _this[elemObj.handle],
                handleOffset = handle.position(),
                handleOffsetLeft = handleOffset.left,
                handleOffsetTop = handleOffset.top,
                angle = _this._valueToAngle(_this.options[elemObj.value]);
            if(angle<=70){
                handleOffsetLeft -=  (parseFloat(_this[elemObj.tooltip].width()) + parseFloat(handle.find('.rs-handle').width()) + (angle/4))
                handleOffsetTop -=  (parseFloat(_this[elemObj.tooltip].height())/2);
                _this[elemObj.tooltip].addClass('left').removeClass('top right');
            }
            else if(angle<=90){
                handleOffsetLeft -=  ((parseFloat(_this[elemObj.tooltip].width())/2)+3);
                handleOffsetTop -=  (parseFloat(_this[elemObj.tooltip].height()) + parseFloat(handle.find('.rs-handle').height()));
                _this[elemObj.tooltip].addClass('top').removeClass('right left');
            }
            else if(angle<=115){
                handleOffsetLeft +=  (parseFloat(handle.width()) - (parseFloat(_this[elemObj.tooltip].width()))- 100);
                //handleOffsetLeft += angle/2;
                handleOffsetTop -=  (parseFloat(_this[elemObj.tooltip].height()) + parseFloat(handle.find('.rs-handle').height()));
                _this[elemObj.tooltip].addClass('top').removeClass('right left');
            }
            else{
                var leftOffset = (angle<135) ? 0 : ( (angle<150) ? 25 : 15);
                handleOffsetLeft +=  (parseFloat(handle.width()) + leftOffset);
                handleOffsetTop -=  (parseFloat(_this[elemObj.tooltip].height())/2);
                _this[elemObj.tooltip].addClass('right').removeClass('left top');
            }

            _this[elemObj.tooltip].css({position:'absolute', top: (handleOffsetTop), left: (handleOffsetLeft), 'z-index': 50});
        }, 100)

    };

    $.fn[pluginName].prototype._roundOffValue = function(){
        if(this.options.roundOff == 10 || this.options.roundOff == 100 || this.options.roundOff == 1000 ){
            this.options.value = Math.round(this.options.value / this.options.roundOff) * this.options.roundOff;
        }
    };
